# Computation of metrics per node - spbdvconf
# input parameters
# - sortByNodeIdInParam
...
# rm spbdvconf_metrics_per_node.txt; x=0; y=0; for (( n=0; n<64; n++ )); do awk  -v xPos=$x -v yPos=$y -v tree=$n -v sortByNodeIdInParam=1 -v sortByBridgeIdInParam=0 -v convTimeAsLastFrwInParam=1 -v convTimeAsLastBpduInParam=0 -v coldStartInParam=1 -v linkFailureInParam=0 -v nodeFailureInParam=0 -f per_node_metrics_spbdvconf.awk n64_grid4_spbDvConf_txHld1_1000000_seq_f35-36_0.txt >> spbdvconf_metrics_per_node.txt; y=`expr $y + 1`;  if [ $y = 8 ];then x=`expr $x + 1`; y=0; fi; done;
...
#########################
/Installing SPBDVconf with bridge/{
			...
			if(bridgeId2nodeId[d[2]]==tree || tree==-1)
				{
					split($2,a,"[");
					split(a[2],b,"]");
					num_confs_rcvd[bridgeId2nodeId[b[1]]]++;
					total_confs_rcvd++;
				}
    }
}
					...
					for( n = 0; n < num_nodes; n++ )
						{
							...
							num_confs_rcvd_final[n]=num_confs_rcvd[n];
							total_confs_rcvd_final=total_confs_rcvd;      
						}
				}
    }
}
...
END{
  ...
  printf("\t%2d\t",total_confs_rcvd_final);
  for( n=0; n < num_nodes; n++ )
    {
      # sort by nodeId
      if(sortByNodeId==1) printf("%2d\t",num_confs_rcvd_final[n]);

      # sort by brigdeId      
      if(sortByBridgeId==1) printf("%2d\t",num_confs_rcvd_final[bridgeId2nodeId[n]]);
    }
