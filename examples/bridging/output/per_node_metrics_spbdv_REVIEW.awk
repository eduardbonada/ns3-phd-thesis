...
# rm per_node_metrics_spbdv.txt; x=0; y=0; for f in `ls -tr n*`;do awk -v xPos=$x -v yPos=$y -v tree=$n -v sortByNodeIdInParam=1 -v sortByBridgeIdInParam=0 -v convTimeAsLastFrwInParam=1 -v convTimeAsLastBpduInParam=0 -v coldStartInParam=1 -v linkFailureInParam=0 -v nodeFailureInParam=0 -f per_node_metrics_spbdv.awk nXX_ >> spdv_metrics_per_node.txt; y=`expr $y + 1`; if [ $y = 8 ];then x=`expr $x + 1`; y=0; fi; done;
