...
public:
  ...
  void SetTraffSink (Ptr<BridgeTraffSink> sink);

  void SetPortState (int64_t tree, uint16_t port, bool forward);

  void ChangeExpirationTime (uint16_t value);
  void FlushEntriesOfPort (uint16_t port);
  ...
  //void SetReceiveStpCallback (NetDevice::ReceiveCallback cb);
...
protected:
  ...
  bool IsPortForwarding (uint16_t port, uint64_t tree);
...
private:
  ...
  //NetDevice::ReceiveCallback m_rxStpCallback; // created by Eduard Bonada
  ...
  std::vector< std::vector< bool > > m_portForwarding; // array for each tree: true if the port is forwarding, false if it is blocked (externally set by xSTP through SetPortState())
  
  Ptr<Node>                     m_node;
  Ptr<BridgeChannel>            m_channel;
  Ptr<BridgeTraffSink>          m_traffSink;
  std::vector< Ptr<NetDevice> > m_ports;
  uint64_t                      m_numNodes;
  uint64_t                      m_bridgeId;    // used to tag incoming frames
