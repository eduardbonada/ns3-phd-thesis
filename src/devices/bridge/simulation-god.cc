  ...
  for (uint64_t i=0 ; i < m_numNodes ; i++ )
    {
      ...
      m_treeWithChangedPortStates.push_back(true);
    }
...
SimulationGod::SetActivePort(uint64_t tree, uint64_t node, uint16_t port, bool forwarding)
{
  ...
  // mark change to execute ReviewTree in next periodical
  m_treeWithChangedPortStates.at(tree)=true;
...
void
SimulationGod::ReviewTree(uint64_t tree)
{
  // check if the flag for this tree is marked
  if(m_treeWithChangedPortStates.at(tree)==true)
    {
      ...
      if(m_stpMode==1 || m_stpMode==2 || m_stpMode==5) // single tree (STP-RSTP-RSTPconf)
        {
          if(m_treeIsSpanning.at(0)!=true && m_failedNodes.at(m_bridgeId2nodeId.at(0))==false) allSpanningTree=false;
        }
      if(m_stpMode==3 || m_stpMode==4 || m_stpMode==6 ) // one tree per node (SPBISIS - SPBDV)  
        {
          for (uint64_t i=0 ; i < m_numNodes ; i++ )
            {
      ...
      m_treeWithChangedPortStates.at(tree)=false;
    }
}
...
void
SimulationGod::CheckSpbIsisPortActivationV1()
{
...
void
SimulationGod::CheckSpbIsisPortActivationV2(uint64_t node, uint16_t port)
{
  // look for the neighbor connected to that port
  for (uint64_t i=0 ; i < m_numNodes ; i++ )
    {
      if( m_portMx.at(node).at(i)==port )
        {
          ...
          // if 'node' and the neighbor have the same top mx
          if(CheckSpbIsIsSameVertAdjMx(node,i))
            {
              // activate the ports of the two node that connect them
               m_spbisiss.at(node)->SetSinglePortToForwarding(true,port);
               m_spbisiss.at(i)->SetSinglePortToForwarding(true,m_portMx.at(i).at(node));
            }
          else
            {
              // deactivate the ports of the two node that connect them
               m_spbisiss.at(node)->SetSinglePortToForwarding(false,port);
               m_spbisiss.at(i)->SetSinglePortToForwarding(false,m_portMx.at(i).at(node));              
            }
