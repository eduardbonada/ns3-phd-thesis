...
public:
   ...
   */
  void Install (Ptr<Node> node, Ptr<BridgeNetDevice> bridge, NetDeviceContainer ports, uint64_t bridgeId, double initTime, uint16_t spbSimulationTimeSecs, uint64_t numNodes, Ptr<SimulationGod> god, uint16_t spbisisActivateBpduForwarding);
...
private:
   ...
   */  
  void SendBPDU(uint16_t outPort);
