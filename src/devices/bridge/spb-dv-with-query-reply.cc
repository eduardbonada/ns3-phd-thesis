  ...
  for (uint16_t i=0 ; i < m_portInfo.size () ; i++)
    {
      ...
      m_portInfo[i].frozen.resize(m_numNodes);
      m_portInfo[i].markedToReply.resize(m_numNodes);
      m_portInfo[i].safeToUse.resize(m_numNodes);
      ...
      for(uint64_t n=0 ; n < m_numNodes ; n++)
        { 
          ...
          // reset the query-reply state
          m_portInfo[i].frozen.at(n)=false;
          m_portInfo[i].markedToReply.at(n)=false;        
          m_portInfo[i].safeToUse.at(n)=false;        
        }
...
SPBDV::ReceiveFromBridge (Ptr<NetDevice> netDevice, Ptr<const Packet> pktBpdu, uint16_t protocol, Address const &src, Address const &dst, NetDevice::PacketType packetType)
{
  ...
  //process if it is a query/reply
  if(bpdu.GetType()==BPDU_QUERY_REPLY)
    { 
      uint64_t tree=bpdu.GetRootIdQueryReply();
      uint16_t qr=bpdu.GetQueryReply();

      if(qr==0) // it is a query
        {          
          NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] SPBDV: Recvs QUERY for tree " << tree);
                   
          // check is was received in a Designated, otherwise exit
          if(m_portInfo[inPort].role.at(tree)!=DESIGNATED_ROLE)
            {
              NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] ERROR: Received a Query in a " << m_portInfo[inPort].role.at(tree) << " port.");
              //exit(1);
            }
          
          //discard it if we have already send a query and are waiting for a reply (check frozen ports)
          bool waitingReply=false;
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++) if(m_portInfo[i].frozen.at(tree)==true) waitingReply=true;
          if(waitingReply==true)
            {
              NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] SPBDV: Discarded query because node is already waiting for a reply");
              
              // mark inPort to reply later
              m_portInfo[inPort].markedToReply.at(tree)=true;            
            }
          else
            {
              // if i am the root send a reply+ to inPort
              if(tree==m_bridgeInfo.bridgeId)
                {
                  SendQueryReply(m_ports[inPort], tree, 1);
                }
                          
              // if i am not the root and the tree is active
              if(tree!=m_bridgeInfo.bridgeId && m_bridgeInfo.activeTree.at(tree)==true)
                {
                  // mark inPort to reply later
                  m_portInfo[inPort].markedToReply.at(tree)=true;
                  
                  // freze and send query to root port and alternates
                  for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
                    {
                      if(m_portInfo[i].role.at(tree)==ROOT_ROLE || m_portInfo[i].role.at(tree)==ALTERNATE_ROLE)
                        {
                          m_portInfo[i].frozen.at(tree)=true;
                          SendQueryReply(m_ports[i], tree, 0);
                        }
                    }
                }
                
              // if the tree is inactive
              if(m_bridgeInfo.activeTree.at(tree)==false)
                {
                  // send a reply- to inPort
                  SendQueryReply(m_ports[inPort], tree, 2);
                }
            }              
        }
      else if(qr==1) // it is a positive reply
        {
          NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] SPBDV: Recvs REPLY+ for tree " << tree);

          // check it was received in a Root/Alt, otherwise exit
          if(m_portInfo[inPort].role.at(tree)==DESIGNATED_ROLE)
            {
              NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] ERROR: Received a Reply+ in a " << m_portInfo[inPort].role.at(tree) << " port.");
              //exit(1);
            }
          ...
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
            {
              // unfreeze inPort (and all others that may have been frozen)
              if(m_portInfo[i].frozen.at(tree)==true)
                {
                  m_portInfo[i].frozen.at(tree)=false;
                  m_portInfo[i].safeToUse.at(tree)=true;                 
                }
                
              // send reply+ to marked ports and unmark them
              if(m_portInfo[i].markedToReply.at(tree)==true)
                {
                  m_portInfo[i].markedToReply.at(tree)=false;
                  SendQueryReply(m_ports[i], tree, 1);
                }     
            }        
                    
          // reconfigure (always? what if i just forwarded a query?)
          ReconfigureBridge(tree);

          // reset safeToUse
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++) if(m_portInfo[i].safeToUse.at(tree)==true) m_portInfo[i].safeToUse.at(tree)=false;                 
      ...
      else if(qr==2) // it is a negative reply
        {
          NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] SPBDV: Recvs REPLY- for tree " << tree);

          // check is was received in a Root/Alt, otherwise exit
          if(m_portInfo[inPort].role.at(tree)==DESIGNATED_ROLE)
            {
              NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] ERROR: Received a Reply- in a " << m_portInfo[inPort].role.at(tree) << " port.");
              //exit(1);
          ...
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
            {
              ...
              // remove info from frozen ports because we are not going to use it
              m_portInfo[i].messAgeTimer.at(tree).Remove();    
              m_portInfo[i].designatedRootId.at(tree) = tree;
              m_portInfo[i].designatedCost.at(tree) = 9999;
              m_portInfo[i].designatedBridgeId.at(tree) = m_bridgeInfo.bridgeId;
              m_portInfo[i].designatedPortId.at(tree) = m_ports[i]->GetIfIndex();
              ...
              if(m_portInfo[inPort].receivedBpdu.at(tree).messAge >= m_bridgeInfo.maxAge /* && m_spbdvActivateExpirationMaxAge == 1*/) // discard BPDU if messAge>maxAge
                {
                 ...
                 //exit(1);  
                 NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "]      Received Message Age higher than MaxAge (MessAge: "<< bpdu.GetMessAge(bpduInd) << ")");
                 MessageAgeExpired(inPort, tree);
                }
                  ...
                  else // normal operation if the cost is <9999
                    {
                      ...
                      if(bpduUpdates == 1 && bpdu.GetRoleFlags(bpduInd)==DESIGNATED_ROLE)
                        {      
                          ...
                          // set safetouse so the reconfigure bridge selects the inPort as possible root
                          m_portInfo[inPort].safeToUse.at(tree)=true;
                                                                
                          //reconfigure the bridge
                          ReconfigureBridge(tree);
                          
                          //unset safetouse for later warnings
                          m_portInfo[inPort].safeToUse.at(tree)=false;
                              ...
                              if(m_bridgeInfo.activeTree.at(tree)==true)
                                {
                                  ...
                                  if(QuickStateTransitions(tree)==true) m_simGod->ReviewTree(tree); 
                                }
      ...
      if(rootAlive == false && allDesignateds==false) // if no more trace of the root, the root of the tree is considered dead
        ...
        { 
          ...
          if(minRootId == m_bridgeInfo.bridgeId)   //computing our tree, so no root port
          ...
          else //computing other trees, select root port
            {
              ...
              for(uint16_t i=0 ; i < m_portInfo.size() ; i++) //from all ports...
                {
                  if(m_portInfo[i].state.at(tree)!=DISABLED_STATE && m_portInfo[i].frozen.at(tree)==0)
                    {
              ...
              if(rootPortTmp == -1) //no minimum root has been found => tree is dead?
              ...
              else
                {
                  ...
                  NS_LOG_LOGIC("ReconfigureBridge: The previous role of the current root port was " << previous_role_rootPort);                   
                  
                  // if the new root port was an alternate and the query-reply mode is active, freeze and send query
                  if( m_spbdvActivateQueryReply==1 && previous_role_rootPort==ALTERNATE_ROLE && m_portInfo[rootPortTmp].safeToUse.at(tree)==false )
                    {                  
                      // freeze new root port (was alternate) and all alternates
                      m_portInfo[rootPortTmp].frozen.at(tree)=true;
                      for(uint16_t i=0 ; i < m_portInfo.size() ; i++) if(m_portInfo[i].role.at(tree)==ALTERNATE_ROLE) m_portInfo[i].frozen.at(tree)=true;
                                           
                      // send query to new root port (was alternate) and all alternates
                      //SendQueryReply(m_ports[rootPortTmp], tree, 0); already sent in next for loop
                      for(uint16_t i=0 ; i < m_portInfo.size() ; i++) if(m_portInfo[i].role.at(tree)==ALTERNATE_ROLE) SendQueryReply(m_ports[i], tree, 0); 
                      
                      // set the root port back
                      m_bridgeInfo.rootPort.at(tree) = previous_root_port;              
                    }
                  else
                    {
                      m_bridgeInfo.rootPort.at(tree) = rootPortTmp;
                      m_portInfo[rootPortTmp].role.at(tree) = ROOT_ROLE;
          ...
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++) //for all ports
            {
              if(m_portInfo[i].state.at(tree)!=DISABLED_STATE && m_bridgeInfo.activeTree.at(tree)==true && m_portInfo[i].frozen.at(tree)==0)
                {
                    ...
                    { 
                      if(m_portInfo[i].designatedRootId.at(tree) != m_bridgeInfo.rootId.at(tree) || m_portInfo[i].designatedBridgeId.at(tree)==m_bridgeInfo.bridgeId ) //not completely updated information, or info from the same bridge
                        { 
                          ...
                          NS_LOG_LOGIC("ReconfigureBridge: port " << i << " elected as " << m_portInfo[i].role.at(tree));
                        }
                      else
                        {
                      ...
                      if(m_portInfo[i].role.at(tree)==DESIGNATED_ROLE) //if designated, edit port variables and save bridge information
                        ...
                        NS_LOG_LOGIC("ReconfigureBridge: Applyng state changes due to role changes. Previous:" << previous_role << " | Current:" << m_portInfo[i].role.at(tree));
                ...
                } // if not disabled, active tree, and not frozen
            } // for all ports...
          ...
          for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
            {
              ...
              NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << p << "]        Port Info  [R:" << m_portInfo[p].designatedRootId.at(tree) << "|C:" << m_portInfo[p].designatedCost.at(tree) << "|B:" << m_portInfo[p].designatedBridgeId.at(tree) << "|P:" << m_portInfo[p].designatedPortId.at(tree) << "|A:" << ageTmp << "|Tc:" << m_portInfo[p].topChange <<  "|Tca:" << m_portInfo[p].topChangeAck << "]:[" << m_portInfo[p].role.at(tree) << "-" << m_portInfo[p].state.at(tree) << "]:[" << (m_portInfo[p].proposed.at(tree)?"P'ed ":"---- ") << (m_portInfo[p].proposing.at(tree)?"P'ng ":"---- ") << (m_portInfo[p].sync.at(tree)?"Sync ":"---- ") << (m_portInfo[p].synced.at(tree)?"S'ed ":"---- ") << (m_portInfo[p].agreed.at(tree)?"A'ed ":"---- ") << (m_portInfo[p].newInfo.at(tree)?"New":"---") << "][Frzn:" << m_portInfo[p].frozen.at(tree) << "|Mrkd:" << m_portInfo[p].markedToReply.at(tree) << "]");   
            }
        }
    
      //check for loops and other stuff
      if(reviewTreeForLoops==true) m_simGod->ReviewTree(tree);           
    }  
}  
...
SPBDV::AllDesignatedsRecentRootExpired(uint64_t tree)
{
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
      if(m_portInfo[p].role.at(tree)==DESIGNATED_ROLE && m_portInfo[p].recentRootTimer.at(tree).IsRunning() && m_portInfo[p].state.at(tree)!=DISABLED_STATE && m_portInfo[p].frozen.at(tree)==0)
        {
          return false;
        }
    }
...
SPBDV::SetSyncToAllRest(uint16_t port, uint64_t tree)
{
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
      if((m_portInfo[p].role.at(tree)==DESIGNATED_ROLE || p!=port) && m_portInfo[p].state.at(tree)!=DISABLED_STATE && m_portInfo[p].frozen.at(tree)==0)
        {
...
SPBDV::AllRestSynced(uint16_t port, uint64_t tree)
{
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
      if( p!= port && !m_portInfo[p].synced.at(tree)  && m_portInfo[p].state.at(tree)!=DISABLED_STATE && m_portInfo[p].frozen.at(tree)==0)
        {
         return false;
        }
    }
...
SPBDV::QuickStateTransitions(uint64_t tree)
{
  ...
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
      ...
      NS_LOG_LOGIC (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << p << "]        Port Info  [R:" << m_portInfo[p].designatedRootId.at(tree) << "|C:" << m_portInfo[p].designatedCost.at(tree) << "|B:" << m_portInfo[p].designatedBridgeId.at(tree) << "|P:" << m_portInfo[p].designatedPortId.at(tree) << "|A:" << ageTmp << "|Tc:" << m_portInfo[p].topChange <<  "|Tca:" << m_portInfo[p].topChangeAck << "]:[" << m_portInfo[p].role.at(tree) << "-" << m_portInfo[p].state.at(tree) << "]:[" << (m_portInfo[p].proposed.at(tree)?"P'ed ":"---- ") << (m_portInfo[p].proposing.at(tree)?"P'ng ":"---- ") << (m_portInfo[p].sync.at(tree)?"Sync ":"---- ") << (m_portInfo[p].synced.at(tree)?"S'ed ":"---- ") << (m_portInfo[p].agreed.at(tree)?"A'ed ":"---- ") << (m_portInfo[p].newInfo.at(tree)?"New":"---") << "][Frzn:" << m_portInfo[p].frozen.at(tree) << "|Mrkd:" << m_portInfo[p].markedToReply.at(tree) << "]");     
    }
  ...
  if(m_bridgeInfo.rootId.at(tree) != m_bridgeInfo.bridgeId) //if we are not root bridge (so there is a root port)
    {
      if(m_portInfo[m_bridgeInfo.rootPort.at(tree)].frozen.at(tree)==0)
        {      
  ...
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
      if(m_portInfo[p].role.at(tree)==DESIGNATED_ROLE && m_portInfo[p].frozen.at(tree)==0)
        {
  ...
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
      if(m_portInfo[p].frozen.at(tree)==0)
        {
      ...
      for( uint64_t t=0 ; t<m_numNodes ; t++ )
        {
          if(m_spbdvActivateBpduMerging==1 || t==tree) // all trees if m_spbdvActivateBpduMerging is activated, or when it is not activated and the t=tree
            {
              NS_LOG_LOGIC("SendBPDU(): Checking MessAge for tree " << t << ": newinfo " << m_portInfo[p].newInfo.at(t) << " - disabled " << (m_portInfo[p].state.at(t)!=DISABLED_STATE) << " - frozen " << (m_portInfo[p].frozen.at(t)==0) );
              if( m_portInfo[p].newInfo.at(t)==true && m_portInfo[p].state.at(t)!=DISABLED_STATE && m_portInfo[p].frozen.at(t)==0)
                {
                  ...
                  if(m_bridgeInfo.rootId.at(t) == m_bridgeInfo.bridgeId)
                    ...
                    {
                      // SPBDV sets the MessAge value exactly to the last value received in the root port + increment
              ...
              for(uint64_t t=0 ; t < m_numNodes ; t++ )
                {
                  if(m_spbdvActivateBpduMerging==1 || t==tree) // all trees if m_spbdvActivateBpduMerging is activated, or when it is not activated and the t=tree
                    {
                      NS_LOG_LOGIC("SendBPDU(): Adding fields of merged BPDU for tree " << t << " (newInfo: " << m_portInfo[p].newInfo.at(t) << " - disabled: " << (m_portInfo[p].state.at(t)!=DISABLED_STATE) << " - frozen " << (m_portInfo[p].frozen.at(t)==0) ); //" - lowerMessAge: " << (messAgeLowerMaxAge.at(t) == true) << 
                      // if the bpdu of the tree 't' is to be merged            
                      if( m_portInfo[p].newInfo.at(t)==true && m_portInfo[p].state.at(t)!=DISABLED_STATE && m_portInfo[p].frozen.at(t)==0 /*&& messAgeLowerMaxAge.at(t) == true*/)
                        {      
                          numBytes = numBytes + 16;
...
///////////////////////////
// SPBDV::SendQueryReply //
///////////////////////////
void
SPBDV::SendQueryReply (Ptr<NetDevice> outPort, uint64_t tree, uint16_t qr)
{
  NS_LOG_FUNCTION_NOARGS ();

  uint16_t p = outPort->GetIfIndex();
  uint64_t numBytes=34 + 8 + 2; //34 ethernet fixed + 8 treeId + 2 Q/R
  
  if (Simulator::Now() < Time(Seconds(m_simulationtime)) )
    {    
      NS_LOG_LOGIC("SendQueryReply(): Port " << p); 
      ...
      // Create BPDU header
      BpduMergedHeader bpduFields = BpduMergedHeader ();
      bpduFields.SetType(BPDU_QUERY_REPLY);                

      // add fields to bpdu header
      bpduFields.SetRootIdQueryReply(tree);
      bpduFields.SetQueryReply(qr);
      ...
      if(qr==0)
        {
          NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Sends QUERY for tree " << tree << " (link: " << outPort->GetChannel()->GetId() << ") (bytes: " << numBytes << " )");
        }
      if(qr==1)
        {
          NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Sends REPLY+ for tree " << tree << " (link: " << outPort->GetChannel()->GetId() << ") (bytes: " << numBytes << " )");
        }
      if(qr==2)
        {
          NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Sends REPLY- for tree " << tree << " (link: " << outPort->GetChannel()->GetId() << ") (bytes: " << numBytes << " )");
...
{
  if(m_portInfo[port].state.at(tree)!=DISABLED_STATE && m_portInfo[port].frozen.at(tree)==0)
    {
  ...
  if(Simulator::Now() < Time(Seconds(m_simulationtime)) && m_spbdvActivateExpirationMaxAge==1 && m_bridgeInfo.activeTree.at(tree)==true)
    {
      ...
      m_portInfo[port].messAge.at(tree) = 20; //m_portInfo[m_bridgeInfo.rootPort.at(tree)].messAge.at(tree);       //??????????
  ...
  if(Simulator::Now() < Time(Seconds(m_simulationtime)))
    {
      ...
      for(uint64_t t=0 ; t < m_numNodes ; t++)
        {
          NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << port << "] SPBDV: Checking Tree " << t);  
          ...
          // check if the node has alternates (if query-reply mode is active)
          bool itHasAlternates=false;
          if(m_spbdvActivateQueryReply==1)
            {
              for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
                {
                  if(m_portInfo[i].role.at(t) == ALTERNATE_ROLE) itHasAlternates=true;
                }
            }
          
          //if the query-reply mode is active, and the root port has failed and it has alternates
          if(m_spbdvActivateQueryReply==1 && m_portInfo[port].role.at(t)==ROOT_ROLE && itHasAlternates==true)
            {
              for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
                {
                  if(m_portInfo[i].role.at(t) == ALTERNATE_ROLE)
                    {                 
                      // freeze alternate port
                      m_portInfo[i].frozen.at(t)=true;

                      // send query to alternate port
                      SendQueryReply(m_ports[i], t, 0);                              
                    }
                }
            }
          // if the query-reply mode is not activated, or it is active but a Des/Alt port has failed or the root port has failed but it has no alternates
          else
            {
