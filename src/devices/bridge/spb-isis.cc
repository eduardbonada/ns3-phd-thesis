    ...
    )
{
  ...
  m_activatePortActivationV1=0; // ideal global activation
  m_activatePortActivationV2=1; // ideal neighbor-neighbor activation
  ...
  for (uint16_t i=0 ; i < m_portInfo.size () ; i++)
    {
      ...
      m_portInfo[i].lspTransmissionIntervalTimer.SetFunction(&SPBISIS::TransmitLspAfterInterval,this);
      m_portInfo[i].pendingLSPs.clear();
      ...
      if(bpdu.GetType()==HELLO_TYPE)
        {
          ...
          //send LSP only if HELLO's have been received in all ports
          //if( m_bridgeInfo.lspDB.at(m_bridgeInfo.bridgeId).adjBridgeId.size()==m_ports.size() )
          //  {
          ...
          if(bpdu.GetLspSequence() < m_bridgeInfo.lspDB.at(bpdu.GetBridgeId()).seq)
            {
              ...
              // send the LSP in my DB
              SendLSP(inPort, bpdu.GetBridgeId());
            }
...
*/      
      ...
      //notify the simGod the adj matrix
      m_simGod->SetSpbIsisTopVertAdjMx(m_node->GetId(), &(m_bridgeInfo.topVertAdjMx));

      //notify the simGod so it can check if all the matrices are the same (PortActivationV1)
      if(m_activatePortActivationV1==1) 
        {
          m_simGod->CheckSpbIsisPortActivationV1();
        }
        
      //notify the simGod so it can check if the neighbor matrices are the same (PortActivationV2)
      else if(m_activatePortActivationV2==1)
        {
          // for each port/neighbor
          for(uint16_t p=0 ; p < m_ports.size() ; p++)
            {
              m_simGod->CheckSpbIsisPortActivationV2(m_node->GetId(),p); // the simGod will activate/deactivate the ports calling to SetSinglePortToForwarding()
            }
        }
      else
        {
          NS_LOG_ERROR("ERROR: SPB::ReceiveFromDevice() portActivationVersion not specified");
          exit(-1);          
        }
      
      //notify the simGod so it can check for loops
      //for(uint64_t t=0 ; t<m_numNodes ; t++) m_simGod->ReviewTree(t);
  ...
  if(m_portInfo[outPort].state.at(0)!=DISABLED_STATE)
    {
      ...
      // send to port
      if(m_lspTransmissionInterval==0 || m_portInfo[outPort].lspTransmissionIntervalTimer.IsRunning()!=true) // if timer since last LSP sent is not running: send
        {
          //tracing
          std::stringstream stringOut;
          uint64_t numBytes=83; //83 fixed and 12 per adjacency            
          ...
          if(m_lspTransmissionInterval!=0)
            { 
              m_portInfo[outPort].lspTransmissionIntervalTimer.Cancel();           
              m_portInfo[outPort].lspTransmissionIntervalTimer.SetArguments(outPort);           
              m_portInfo[outPort].lspTransmissionIntervalTimer.Schedule(Seconds(m_lspTransmissionInterval)); 
            }
        }
      else // if the LSP needs to be held: the timer expiration function will send it later on
        {
          ...
          m_portInfo[outPort].pendingLSPs.push_back(lspNodeId);       
        }
    }
}
...
void SPBISIS::TransmitLspAfterInterval(uint16_t port)
{
  ...
  if(m_portInfo[port].pendingLSPs.size()>0) // if there is a pending LSP to send
    {
      //take the LSPId to send (the first pending)
      uint64_t lspNodeId=m_portInfo[port].pendingLSPs.at(0);
      m_portInfo[port].pendingLSPs.erase(m_portInfo[port].pendingLSPs.begin());
...
//////////////////////////////////////////
// SPBISIS::SetSinglePortToForwarding() //
//////////////////////////////////////////
void
SPBISIS::SetSinglePortToForwarding(bool forw, uint16_t port)
{

  if(forw==true)
    {
      // set root and designated ports to forwarding
      for(uint64_t t=0 ; t<m_numNodes ; t++)
        {
          if( m_portInfo[port].role.at(t) == ROOT_ROLE || m_portInfo[port].role.at(t) == DESIGNATED_ROLE )
            {
              if(m_portInfo[port].state.at(t) != FORWARDING_STATE) MakeForwarding(port,t);
            }
        }  
    }
  else
    {
      // set ports to discarding
      for(uint64_t t=0 ; t<m_numNodes ; t++)
        { 
          if( m_portInfo[port].role.at(t) == ROOT_ROLE || m_portInfo[port].role.at(t) == DESIGNATED_ROLE )
            {
              if(m_portInfo[port].state.at(t) != DISCARDING_STATE) MakeDiscarding(port,t);
            }
        }
    }
}
...
void SPBISIS::PhyLinkFailureDetected(uint16_t port)
{
  if(Simulator::Now() < Time(Seconds(m_simulationtime)))
    { 
      ...
      //notify the simGod so it can check if all the matrices are the same (PortActivationV1)
      if(m_activatePortActivationV1==1) m_simGod->CheckSpbIsisPortActivationV1();
      ...
      //notify the simGod so it can check if the neighbor matrices are the same (PortActivationV2)
        ...
        {
          NS_LOG_ERROR("ERROR: SPB::PhyLinkFailureDetected() portActivationVersion not specified");
          exit(-1);          
        }
