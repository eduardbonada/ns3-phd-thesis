          ...
          if(m_portInfo[p].txHoldCount < m_bridgeInfo.maxBpduTxHoldPeriod) 
            {
              ...
              // debug message              
              std::stringstream stringOut;
              stringOut << Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Sends BPDU with " << numBpdusMerged << " merged BPDUs (link: " << outPort->GetChannel()->GetId() << ") (bytes: " << numBytes << " ) (" << debugComment << ")";
              NS_LOG_INFO(stringOut.str());
