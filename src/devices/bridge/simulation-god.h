...
public:
  ...
  void CheckSpbIsisPortActivationV1();
  void CheckSpbIsisPortActivationV2(uint64_t, uint16_t);
...
private:  
  ...
  std::vector<bool> m_treeWithChangedPortStates;  // to monitor if there has been a change in that tree from last ReviewTree (to avoid checking in all periodical)
};
