/*
THIS VERSION UPDATES THE ORIGINAL CONF BY INTRODUCING 'RD' MESSAGES THAT DO NOT NEED 
TO BE REPLIED BY THE ROOT BUT ONLY USED FOR FLOODING THE ALARM. 
A NODE WITHOUT ALTERNATE PORTS FLOODS THESE RD MESSAGES (FOR THE ALARM IN CASE THE ROOT DIED),
BUT ALSO ARISES AS ROOT SENDING BPDUS.
A NODE WITH ALTERNATE PORTS FLOODS RD MESSAGES THAT MUS BE REPLIED BY AN 'RA'.
*/
...
private:
  ...
  bool                     m_iAmNeighbor;           // boolean indicating whether  the node is a neighbor or not
  uint16_t                 m_numNeighbors;          // number of neighbors. disseminated by the root node (numPorts of Root)
  std::vector< bool >      m_rdRcvdFromNeigh;       // array containing whether the 'rd?' has been received for a given neighbor. index is local neighbor number.
  std::vector< uint16_t >  m_rdRcvdInPort;          // array containing the local portId where the 'rd?' has benn received. index is local neighbor number.
  std::vector< uint64_t >  m_neighId;               // array containing the bridgeId of the neighbor that corresponds to the given index. index is local neighbor number.

  void SendConfirmationBpdu(Ptr<NetDevice> , uint16_t , uint64_t , uint64_t );
  ///////////////////////////////////////////
