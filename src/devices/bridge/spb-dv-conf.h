...
#define BPDU_PV_MERGED_TYPE 600 
#define BPDU_CONF_TYPE 700 // BPDU to confirm root alive
#define BPDU_PV_MERGED_CONF_TYPE 800
...
public:
  ...
  typedef struct{
    ...
    std::vector<std::vector < uint64_t > >   pathVector;
    std::vector<uint16_t>         dissemFlag;                 //marked if the BPDU has to be sent with the disseminate flag set (forces dissemination after 'equal from designated')

    std::vector<bool>         receivedTreeDeactivation;   // has received a BPDU with cost '9999'
    std::vector<std::string>  consolidatedRole;           // the port roles that are consolidated after a tree configuraion (a timer sets them)    
    std::vector<Timer>        setConsolidatedRolesTimer;  // timer triggered after Xsec of the tree configuration (after each transition to FRW)
  }portInfo_t;
...
private:
  ...
  bool QuickStateTransitions(uint64_t tree);
  ...
  void SetConsolidatedRoles(uint64_t tree);
  ...
  uint16_t  m_spbdvActivateExpirationMaxAge;      // we can also avoid expiring max age timers and avoid steady state recalculations (this is the BPDU ageing)
  ...
  uint16_t  m_spbdvActivateTreeDeactivationV2;
  uint16_t  m_spbdvActivateTreeDeactivationV3;
  uint16_t  m_spbdvActivateFloodingTowardsRoot;
  uint16_t  m_spbdvActivateMessAgeDiscarding;     // true: discard the BPDU with MA=MaxAge, false: remove port info and reconfigure (early bpdu ageing)
  uint16_t  m_spbdvActivateSimGodOftenCalls;
  ...
  uint16_t                              m_activateRootFailureConf;
  uint16_t  m_rootSendsRa;                                       // true: root sends 'ra' in reply to 'rd'; false: root sends BPDU
  std::vector<bool>                     m_iAmNeighbor;           // boolean indicating whether  the node is a neighbor or not
  std::vector<uint16_t>                 m_numNeighbors;          // number of neighbors. disseminated by the root node (numPorts of Root)
  std::vector<std::vector< bool> >      m_rdRcvdFromNeigh;       // array containing whether the 'rd?' has been received for a given neighbor. index is local neighbor number.
  std::vector<std::vector< uint16_t> >  m_rdRcvdInPort;          // array containing the local portId where the 'rd?' has benn received. index is local neighbor number.
  std::vector<std::vector< uint64_t> >  m_neighId;               // array containing the bridgeId of the neighbor that corresponds to the given index. index is local neighbor number.

  void SendConfirmationBpdu(Ptr<NetDevice> outPort, uint16_t type, uint64_t rootId, uint64_t neighId);
  ///////////////////////////////////////////
