    ...
    )
{
  ...
  m_activateLspSubstitution=false;
  ...
  for (NetDeviceContainer::Iterator i = ports.Begin (); i != ports.End (); ++i)
    {
      ...
      NS_LOG_INFO ("Node[" << m_node->GetId()portQueues << "] SPBISIS: RegisterProtocolHandler for " << (*i)->GetIfIndex());
      ...
      // add queue for LSPs
      Ptr<Queue> q = m_queueFactory.Create<Queue> ();
      m_portQueues.push_back(q);
    }
  ...
  // temporary promise to init port values
  ...
  for (uint16_t i=0 ; i < m_portInfo.size () ; i++)
    {
      ...
      m_portInfo[i].lspTransmissionInterval.SetFunction(&SPBISIS::TransmitLspAfterInterval,this);
      m_portInfo[i].lspTransmissionInterval.SetArguments(i);  
      //m_portInfo[i].lspTransmissionInterval.Schedule(Seconds(m_lspTransmissionInterval)); 
    }
...
///////////////////////////////////////////////////////////////////////////////////////
// I should call a function like SendLSP(port,LSPId) where LSPId it is me
///////////////////////////////////////////////////////////////////////////////////////
              SendLSP(inPort);
            }
              ...
              for(uint16_t p=0 ; p < m_ports.size() ; p++)
                {
                  if(p!=inPort && m_portInfo[p].state.at(0)!=DISABLED_STATE)  // the port is disabled for all trees, so checking for tree 0 is enough
                    {
///////////////////////////////////////////////////////////////////////////////////////
// this should go in a function like SendLSP(port,LSPId)
///////////////////////////////////////////////////////////////////////////////////////
//
//
//
                      ...
                      if(m_portInfo[p].lspTransmissionInterval.IsRunning()!=true)
                        {
                          //tracing
                          std::stringstream stringOut;                
                          ...
                          // to the queue
                          m_portInfo[p].lspTransmissionInterval.Schedule(Seconds(m_lspTransmissionInterval)); 
                          m_portQueues[p]->Enqueue (lspFlood);
                          lspFlood = m_portQueues[p]->Dequeue ();
                          m_ports[p]->SendFrom (lspFlood, src, dst, protocol);
                        }
                      else // if the LSP needs to be held: just enqueue
                        {
                          //tracing
                          std::stringstream stringOut;                
                          uint64_t numBytes=83; // 83 fixed plus 12 per adjacency
                          stringOut << Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << p << "] SPBISIS: Enqueues LSP   [b:" << bpdu.GetBridgeId() << "|s:" << bpdu.GetLspSequence() << "|a:" << bpdu.GetLspNumAdjacencies() << "]=>";
                          for (uint64_t i=0 ; i < bpdu.GetLspNumAdjacencies() ; i++ )
                            {
                              stringOut << "[b:" << bpdu.GetAdjBridgeId(i) << "|c:" << bpdu.GetAdjCost(i) << "]";
                              numBytes = numBytes + 12;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// this function should have arguments like SendLSP(port,LSPId), and select an LSP from database
///////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(m_portInfo[outPort].state.at(0)!=DISABLED_STATE)
    {
      ...
      // send to port
      if(m_portInfo[outPort].lspTransmissionInterval.IsRunning()!=true) // if timer since last LSP sent is not running: enqueue+dequeue+send
        {
          //tracing
          std::stringstream stringOut;
          uint64_t numBytes=83; //83 fixed and 12 per adjacency            
          ...
          // to the queue
          m_portInfo[outPort].lspTransmissionInterval.Schedule(Seconds(m_lspTransmissionInterval)); 
          m_portQueues[outPort]->Enqueue (lsp);
          lsp = m_portQueues[outPort]->Dequeue ();
      ...
      else // if the LSP needs to be held: just enqueue
        {
          ...
          std::stringstream stringOut;
          uint64_t numBytes=83; //83 fixed and 12 per adjacency            
          stringOut << Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort << "] SPBISIS: Enqueues LSP   [b:" << lspFields.GetBridgeId() << "|s:" << lspFields.GetLspSequence() << "|a:" << lspFields.GetLspNumAdjacencies() << "]=>";
          for (uint64_t i=0 ; i < m_bridgeInfo.lspDB.at(m_bridgeInfo.bridgeId).adjBridgeId.size() ; i++ )
            {
              stringOut << "[b:" << m_bridgeInfo.lspDB.at(m_bridgeInfo.bridgeId).adjBridgeId.at(i) << "|c:" << m_bridgeInfo.lspDB.at(m_bridgeInfo.bridgeId).adjCost.at(i) << "]";
              numBytes = numBytes + 12;
...
///////////////////////////////////////
void SPBISIS::TransmitLspAfterInterval(uint16_t port)
{
  if(m_portQueues[port]->GetNPackets()>=1) // if there is at least one LSP in the queue
    {
      // dequeue LSP
      Ptr<Packet> lsp = m_portQueues[port]->Dequeue ();
      uint16_t protocol = 9999; //should be the LLC protocol
      Mac48Address src = Mac48Address("AA:AA:AA:AA:AA:AA");// whatever, testing, should be the management MAC of the sending bridge ?
      Mac48Address dst = Mac48Address("01:80:C2:00:00:00");
      ...
      //tracing
      BpduHeader lspFields = BpduHeader ();
      Ptr<Packet> p = lsp->Copy ();
      p->RemoveHeader(lspFields);

      std::stringstream stringOut;                
      uint64_t numBytes=83; // 83 fixed plus 12 per adjacency
      stringOut << Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << port << "] SPBISIS: Sends LSP   [b:" << lspFields.GetBridgeId() << "|s:" << lspFields.GetLspSequence() << "|a:" << lspFields.GetLspNumAdjacencies() << "]=>";
      for (uint64_t i=0 ; i < lspFields.GetLspNumAdjacencies() ; i++ )
        {
          stringOut << "[b:" << lspFields.GetAdjBridgeId(i) << "|c:" << lspFields.GetAdjCost(i) << "]";
          numBytes = numBytes + 12;
        }
      NS_LOG_INFO(stringOut.str() << " (link:" << m_ports[port]->GetChannel()->GetId() << ") (bytes: " << numBytes << " )");

      //send to port
      m_ports[port]->SendFrom (lsp, src, dst, protocol);

      //configure next lsp rate limitation timer
      if(m_portQueues[port]->GetNPackets()>=1)
        {
          m_portInfo[port].lspTransmissionInterval.SetArguments(port);  
          m_portInfo[port].lspTransmissionInterval.Schedule(Seconds(m_lspTransmissionInterval));
        }
    }
