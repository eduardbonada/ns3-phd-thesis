  ...
  if(bpdu.GetType()==BPDU_CONF_TYPE)
    {
                  ...
                  if(m_bridgeInfo.rootId.at(tree) == m_bridgeInfo.bridgeId)
                    {
                        ...
                        else //root sends BPDU
                          {
                            m_portInfo[inPort].newInfo.at(tree) = true;
                            SendBPDU(m_ports[inPort], "rd_reply_BPDU", tree);
                          }         
                    }
                  else
                    {
      ...
      ////////////////////////////
      else if(bpdu.GetConfType()==1) // received 'ra'
        {
          ...
          else // Root has sent a BPDU, so no 'ra' is never received
            {
              // ADD HERE A SANITY CHECK AND EXIT IF AN 'RA' IS RECEIVED
            }       
        }
      else
        {
    ...
    //if(duplicatedPathVector==true) bpduUpdates=-1;
                      ...
                      if(bpduUpdates == 0 && bpdu.GetRoleFlags(bpduInd)==DESIGNATED_ROLE)  
                        {       
                          ...
                          if(m_activateRootFailureConf==1)
                            {
                              // send equal BPDU to port where 'rd' was received
                              if(m_rootSendsRa==0)
                                {
                                  std::vector<uint16_t> alreadySentToPort;
                                  alreadySentToPort.resize(m_portInfo.size());
                                  for(uint16_t i=0 ; i < m_portInfo.size() ; i++) alreadySentToPort[i]=0;

                                  //check if an 'rd' is pending to be replied
                                  for(uint16_t neighTmp=0 ; neighTmp < m_rdRcvdFromNeigh.at(tree).size() ; neighTmp++)
                                    {
                                      if(m_rdRcvdFromNeigh.at(tree).at(neighTmp) == true)
                                        {
                                          if(alreadySentToPort[m_rdRcvdInPort.at(tree).at(neighTmp)]==false)
                                            {
                                              m_portInfo[m_rdRcvdInPort.at(tree).at(neighTmp)].newInfo.at(tree) = true;
                                              SendBPDU(m_ports[m_rdRcvdInPort.at(tree).at(neighTmp)], "rd_reply_BPDU", tree);
                                            }
                                          alreadySentToPort[m_rdRcvdInPort.at(tree).at(neighTmp)]=true;
                                        }
                                    }
                                }
