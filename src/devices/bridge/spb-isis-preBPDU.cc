  ...
  if(m_portInfo[inPort].state.at(0)==DISABLED_STATE) // the port is disabled for all trees, so checking for tree 0 is enough
  ...
  else
    {
      ...
      if(bpdu.GetType()==HELLO_TYPE)
        {
          NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] SPBISIS: Recvs HELLO [b:" << bpdu.GetBridgeId() << "|c:" << bpdu.GetHelloCost() << "]");        
      ...
      //////////////////////////////////////////////
      else if (bpdu.GetType()==LSP_TYPE)
        {
          ...
          NS_LOG_INFO(stringOut.str() << " - (link id: " << m_ports[inPort]->GetChannel()->GetId() << ")");
                  ...
                  if(p!=inPort && m_portInfo[p].state.at(0)!=DISABLED_STATE)  // the port is disabled for all trees, so checking for tree 0 is enough
                    {
                      ...
                      NS_LOG_INFO(stringOut.str() << " - (link id: " << m_ports[p]->GetChannel()->GetId() << ")");
      ...
      //////////////////////////////////////////////
        ...
        {          
          NS_LOG_ERROR("ERROR: SPB::ReceiveFromDevice() received BPDU type when not implemented...");
          exit(-1);
...
SPBISIS::ReconfigureBridge ()
{
  ...
  */
  ...
  //if not adjacency list size does not match the number of not-Disabled ports (not all hellos received for example)
  uint16_t counter=0;
  for(uint16_t p=0 ; p < m_ports.size() ; p++)
    {
...
*/      
      ...
      m_simGod->SetSpbIsisTopVertAdjMx(m_bridgeInfo.bridgeId, &(m_bridgeInfo.topVertAdjMx));
      ...
      if(m_portInfo[p].state.at(0)!=DISABLED_STATE)
        {
          ...
          NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << p << "] SPBISIS: Sends HELLO [b:" << helloFields.GetBridgeId() << "|c:" << helloFields.GetHelloCost() << "]");
  ...
  if(m_portInfo[outPort].state.at(0)!=DISABLED_STATE)
    {
      ...
      NS_LOG_INFO(stringOut.str() <<" - (link id: " << m_ports[outPort]->GetChannel()->GetId() << ")");
  ...
  for (uint64_t n=0 ; n < m_numNodes ; n++ )
    {
      ...
      for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
        {        
          stringOut << "    P[" << p << "] [R:" << m_portInfo[p].designatedRootId.at(n) << "|C:" << m_portInfo[p].designatedCost.at(n) << "|B:" << m_portInfo[p].designatedBridgeId.at(n) << "]:[" << m_portInfo[p].role.at(n) << "-" << m_portInfo[p].state.at(n) << "]\n";   
        }
