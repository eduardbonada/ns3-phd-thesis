  ...
  for (uint16_t i=0 ; i < m_portInfo.size () ; i++)
    {
      ...
      for(uint64_t n=0 ; n < m_numNodes ; n++)
        { 
          ...
          m_portInfo[i].newInfo.push_back(true);
          ...
          m_portInfo[i].messAgeTimer.at(n).SetDelay(Seconds(3*m_bridgeInfo.helloTime)); //in SPBDV(RSTP), the value does not directly depend on the received MessAge (it is either 3xHelloTime, if       
...
SPBDV::ReceiveFromBridge (Ptr<NetDevice> netDevice, Ptr<const Packet> pktBpdu, uint16_t protocol, Address const &src, Address const &dst, NetDevice::PacketType packetType)
{
  ...
  uint64_t numBytes = 70; //70 bytes fixed + 10 per PV element
  stringOut << Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] SPBDV: Recvs BPDU [R:" << bpdu.GetRootId() << "|C:" << bpdu.GetCost() << "|B:" << bpdu.GetBridgeId() << "|P:" << bpdu.GetPortId() << "|A:" << bpdu.GetMessAge() << "|Tc:" << bpdu.GetTcFlag() << "|Tca:" << bpdu.GetTcaFlag() << "|" << bpdu.GetRoleFlags() << (bpdu.GetPropFlag()?"P":"-") << (bpdu.GetAgrFlag()?"A":"-") << (bpdu.GetLrnFlag()?"L":"-") << (bpdu.GetFrwFlag()?"F":"-") << "] PV(" << bpdu.GetPathVectorNumBridgeIds() << ")[";
  for(uint16_t i=0 ; i < bpdu.GetPathVectorNumBridgeIds() ; i++)
    {
      stringOut << bpdu.GetPathVectorBridgeId(i);
      if(i<bpdu.GetPathVectorNumBridgeIds()-1) stringOut << ">";
      
      pathVectorTmp[bpdu.GetPathVectorBridgeId(i)]++;
      if(pathVectorTmp[bpdu.GetPathVectorBridgeId(i)] > 1) duplicatedPathVector=true;
      
      numBytes = numBytes + 8;
    }    
  stringOut << "] (link: " << m_ports[inPort]->GetChannel()->GetId() << ") (bytes: " << numBytes << " )";
  ...
  m_portInfo[inPort].receivedBpdu.at(tree).rootId = bpdu.GetRootId();
  m_portInfo[inPort].receivedBpdu.at(tree).cost = bpdu.GetCost();
  m_portInfo[inPort].receivedBpdu.at(tree).bridgeId = bpdu.GetBridgeId();  
  m_portInfo[inPort].receivedBpdu.at(tree).portId = bpdu.GetPortId();
  m_portInfo[inPort].receivedBpdu.at(tree).messAge = bpdu.GetMessAge();
  ...
  if(duplicatedPathVector==true && m_spbdvActivatePathVectorDuplDetect==1)
    ...
    {      
      int16_t bpduUpdates = UpdatesInfo(bpdu, inPort, tree);
      ...
      if(bpduUpdates == 1 && bpdu.GetRoleFlags()==DESIGNATED_ROLE)
        {      
          ...
          if(bpdu.GetPropFlag()) m_portInfo[inPort].proposed.at(tree)  = true;
          ...
          for(uint16_t i=0 ; i < bpdu.GetPathVectorNumBridgeIds() ; i++)
            {
              m_portInfo[inPort].pathVector.at(tree).push_back(bpdu.GetPathVectorBridgeId(i));
            }
          ...
          CheckProcessTcFlags(bpdu.GetTcaFlag(), bpdu.GetTcFlag(), inPort, m_portInfo[inPort].role.at(tree));
      ...
      if( bpduUpdates == -1 && ( bpdu.GetRoleFlags()==ROOT_ROLE || bpdu.GetRoleFlags()==ALTERNATE_ROLE ) && bpdu.GetAgrFlag() && bpdu.GetRootId()==m_bridgeInfo.rootId.at(tree) )
        {
          //traces
...
////////////////////
int16_t
SPBDV::UpdatesInfo(BpduHeader bpduHdr, uint16_t port, uint64_t tree)
{
  NS_LOG_FUNCTION_NOARGS ();
...
*/
  ...
  //////////////////////////
  NS_LOG_LOGIC("UpdatesInfo: BPDU [R:" << bpduHdr.GetRootId() << "|C:" << bpduHdr.GetCost() << "|B:" << bpduHdr.GetBridgeId() << "|P:" << bpduHdr.GetPortId() << "] <=> Port[" << port << "]: [R:" << m_portInfo[port].designatedRootId.at(tree) << "|C:" << m_portInfo[port].designatedCost.at(tree) << "|B:" << m_portInfo[port].designatedBridgeId.at(tree) << "|P:" << m_portInfo[port].designatedPortId.at(tree) << "]");

  std::stringstream stringOut;
  if(bpduHdr.GetPathVectorNumBridgeIds() == 0) // no path vector received, this is a BPDU going up with agreement
    {
      stringOut << "BPDU without path vector";
    }
  else
    {
      stringOut << "BPDU Path Vector    (" << bpduHdr.GetPathVectorNumBridgeIds() << "): [";
      for(uint16_t i=0 ; i < bpduHdr.GetPathVectorNumBridgeIds() ; i++)
        {
          stringOut << bpduHdr.GetPathVectorBridgeId(i);
          if(i<bpduHdr.GetPathVectorNumBridgeIds()-1) stringOut << ">";
        }    
      stringOut << "]\n";
  ...
  // start comparing
  ////////////////////      
  if(bpduHdr.GetRootId() < m_portInfo[port].designatedRootId.at(tree))
    {
      ///////////////////////////////////////////////////////////
      // I THINK WE NEVER ENTER HERE, PUT AN ERROR AND CHECK
      ///////////////////////////////////////////////////////////
    ...
    {
      if(bpduHdr.GetRootId() == m_portInfo[port].designatedRootId.at(tree) && bpduHdr.GetCost() < m_portInfo[port].designatedCost.at(tree))
        {
          returnValue = 1;
        }
      else
        {
          if(bpduHdr.GetRootId() == m_portInfo[port].designatedRootId.at(tree) && bpduHdr.GetCost() == m_portInfo[port].designatedCost.at(tree) && BetterPathVector(true, bpduHdr, port, 9999, tree)==1)
            {
              returnValue = 1;
            }
          else
            {
              if(bpduHdr.GetRootId() == m_portInfo[port].designatedRootId.at(tree) && bpduHdr.GetCost() == m_portInfo[port].designatedCost.at(tree) && BetterPathVector(true, bpduHdr, port, 9999, tree)==0)
                {
                  returnValue = 0;
                }
            }
  ...
  if(returnValue==2)  
    {
      ...
      if(bpduHdr.GetBridgeId() == m_portInfo[port].designatedBridgeId.at(tree) &&  bpduHdr.GetPortId() == m_portInfo[port].designatedPortId.at(tree) && m_portInfo[port].designatedBridgeId.at(tree)!=m_bridgeInfo.bridgeId )
        {
...
///////////////////////////
int16_t
SPBDV::BetterPathVector(bool bpduRcvd, BpduHeader bpduHdr, uint16_t portA, uint16_t portB, uint64_t tree)
{
  ...
  if(bpduRcvd==true)
    {
      ...
      if(bpduHdr.GetRootId() == m_bridgeInfo.bridgeId) 
        {
      ...
      pv1.resize(bpduHdr.GetPathVectorNumBridgeIds());
      for(uint16_t i=0 ; i < bpduHdr.GetPathVectorNumBridgeIds() ; i++)
        {
          pv1.at(i) = bpduHdr.GetPathVectorBridgeId(i);
        }
...
SPBDV::ReconfigureBridge(uint64_t tree)
{
  ...
  // activate tree
  if(m_bridgeInfo.activeTree.at(tree) == false) NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[-]       Activate tree " << tree);
  ...
  BpduHeader BpduHdrTmp = BpduHeader ();
                  ...
                  if(m_portInfo[i].designatedRootId.at(tree) == m_bridgeInfo.rootId.at(tree) && m_portInfo[i].designatedBridgeId.at(tree)!=m_bridgeInfo.bridgeId) //...that are aware of the right minimum root and are not one of the designated ports (because these have local info)
                    { 
                      if(m_portInfo[i].designatedCost.at(tree) < minCost) // cost is lower
                        ...
                        {
                          if(m_portInfo[i].designatedCost.at(tree) == minCost && BetterPathVector(false, BpduHdrTmp, i, (uint16_t)rootPortTmp, tree)==1) // cost is equal and path vector of port i is better than pathVector in rootPortTmp
                            {
                              rootPortTmp = i;
                              minCost = m_portInfo[i].designatedCost.at(tree);
                              minBridgeId = m_portInfo[i].designatedBridgeId.at(tree);
                  ...
                  else
                    {
                      if(m_portInfo[i].designatedCost.at(tree) > m_bridgeInfo.cost.at(tree)) // if cost stored in port is larger than bridge cost, designated port
                      ...
                      else
                        {
                          ...
                          if(m_portInfo[i].designatedCost.at(tree) == m_bridgeInfo.cost.at(tree) && BetterPathVector(false, BpduHdrTmp, i, m_bridgeInfo.rootPort.at(tree), tree)==-1 ) // if cost is equal and path vector stored in the port is worse than local (root port)
                            {
                              previous_role = m_portInfo[i].role.at(tree);
                              m_portInfo[i].role.at(tree)=DESIGNATED_ROLE;
                            }
                            ...
                            {
                              if(m_portInfo[i].designatedCost.at(tree)  == m_bridgeInfo.cost.at(tree) && BetterPathVector(false, BpduHdrTmp, i, m_bridgeInfo.rootPort.at(tree), tree)==1 ) // if cost is equal and path vector stored in the port is better than local (root port)
                                {
                                  previous_role = m_portInfo[i].role.at(tree);
                                  m_portInfo[i].role.at(tree)=ALTERNATE_ROLE;
                                }
                              if(m_portInfo[i].designatedCost.at(tree)  == m_bridgeInfo.cost.at(tree) && BetterPathVector(false, BpduHdrTmp, i, m_bridgeInfo.rootPort.at(tree), tree)==0 ) // cost and path vector are equal
                                {
                                  previous_role = m_portInfo[i].role.at(tree);
                                  m_portInfo[i].role.at(tree)=DESIGNATED_ROLE;                                  
                                }
...
SPBDV::SendBPDU (Ptr<NetDevice> outPort, std::string debugComment, uint64_t tree)
{
  ...
  NS_LOG_LOGIC("SendBPDU: disseminate to port " << p << " in tree " << tree << "? =>  Port State [R:" << m_portInfo[p].designatedRootId.at(tree) << "|C:" << m_portInfo[p].designatedCost.at(tree) << "|B:" << m_portInfo[p].designatedBridgeId.at(tree) << "|P:" << m_portInfo[p].designatedPortId.at(tree) << "]:[" << m_portInfo[p].role.at(tree) << "-" << m_portInfo[p].state.at(tree) << "]:[" << (m_portInfo[p].proposed.at(tree)?"P'ed ":"---- ") << (m_portInfo[p].proposing.at(tree)?"P'ng ":"---- ") << (m_portInfo[p].sync.at(tree)?"Sync ":"---- ") << (m_portInfo[p].synced.at(tree)?"S'ed ":"---- ") << (m_portInfo[p].agreed.at(tree)?"A'ed ":"---- ") << (m_portInfo[p].newInfo.at(tree)?"New":"---") << "]");  
     
  if(Simulator::Now() < Time(Seconds(m_simulationtime)) && m_portInfo[p].newInfo.at(tree)==true)
    {
      if(m_portInfo[p].state.at(tree)!=DISABLED_STATE)
        {
          if(m_portInfo[p].txHoldCount < m_bridgeInfo.maxBpduTxHoldPeriod) //if no BPDUs send during last period, we can safley sent
            {
              ...
              bpduFields.SetType(BPDU_PV_TYPE);
              bpduFields.SetRootId(m_portInfo[p].designatedRootId.at(tree));
              bpduFields.SetCost(m_bridgeInfo.cost.at(tree));//m_portInfo[p].designatedCost);
              bpduFields.SetBridgeId(m_bridgeInfo.bridgeId);//m_portInfo[p].designatedBridgeId);
              bpduFields.SetPortId(m_portInfo[p].designatedPortId.at(tree));
              
              //set flags
              if(m_portInfo[p].proposing.at(tree)) bpduFields.SetPropFlag(true); else bpduFields.SetPropFlag(false);
              bpduFields.SetRoleFlags(m_portInfo[p].role.at(tree));
              if(m_portInfo[p].state.at(tree)==DISCARDING_STATE) bpduFields.SetLrnFlag(false); else bpduFields.SetLrnFlag(true);
              if(m_portInfo[p].state.at(tree)==FORWARDING_STATE) bpduFields.SetFrwFlag(true); else bpduFields.SetFrwFlag(false);          
              if(m_portInfo[p].synced.at(tree) && (m_portInfo[p].role.at(tree)==ROOT_ROLE || m_portInfo[p].role.at(tree)==ALTERNATE_ROLE) ) bpduFields.SetAgrFlag(true); else bpduFields.SetAgrFlag(false);
              bpduFields.SetTcFlag(m_portInfo[p].topChange);          
              bpduFields.SetTcaFlag(m_portInfo[p].topChangeAck); 
              
              // set the path vector
              if(m_portInfo[p].role.at(tree)==DESIGNATED_ROLE) // path vectors only added in BPDUs following Des ports
                {
                  if(m_bridgeInfo.rootId.at(tree) == m_bridgeInfo.bridgeId) //if the local node id the root of the tree
                    {
                      //add the local bridgeId as the path vector
                      bpduFields.SetPathVectorNumBridgeIds(1);
                      bpduFields.AddPathVectorBridgeId(m_bridgeInfo.bridgeId);
                    }
                  else
                    {
                      //add the path vector in the bridge
                      bpduFields.SetPathVectorNumBridgeIds( m_bridgeInfo.pathVector.at(tree).size() );
                      for( uint16_t i=0 ; i < m_bridgeInfo.pathVector.at(tree).size() ; i++)
                        {
                          bpduFields.AddPathVectorBridgeId( m_bridgeInfo.pathVector.at(tree).at(i) ); 
                        }
                    }
                }
              
              //set MessAge
              if(m_bridgeInfo.rootId.at(tree) == m_bridgeInfo.bridgeId || m_bridgeInfo.rootPort.at(tree)==-1 ) bpduFields.SetMessAge(0); // it is the root
              else
                {
                  // SPBDV sets the MessAge value exactly to the last value received in the root port
                  if(m_spbdvActivateIncreaseMessAge==1)
                    {
                      bpduFields.SetMessAge( (uint16_t) ( m_portInfo[m_bridgeInfo.rootPort.at(tree)].messAge.at(tree) + m_bridgeInfo.messAgeInc ) ); // it is not the root
                    }
                  else
                    {
              ...
              if(bpduFields.GetMessAge() < m_bridgeInfo.maxAge)
                {
                  ...
                  // debug message
                  uint64_t numBytes=70; //70 fixed + 8 per path vector element
                  std::stringstream stringOut;
                  stringOut << Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Sends BPDU [R:" << bpduFields.GetRootId() << "|C:" << bpduFields.GetCost() << "|B:" << bpduFields.GetBridgeId() << "|P:" << bpduFields.GetPortId() << "|A:" << bpduFields.GetMessAge() << "|Tc:" << bpduFields.GetTcFlag() << "|Tca:" << bpduFields.GetTcaFlag() << "|" << bpduFields.GetRoleFlags() <<(bpduFields.GetPropFlag()?"P":"-") << (bpduFields.GetAgrFlag()?"A":"-") << (bpduFields.GetLrnFlag()?"L":"-") << (bpduFields.GetFrwFlag()?"F":"-") << "] PV(" << bpduFields.GetPathVectorNumBridgeIds() << ")[";
                  for(uint16_t i=0 ; i < bpduFields.GetPathVectorNumBridgeIds() ; i++)
                    {
                      stringOut << bpduFields.GetPathVectorBridgeId(i);
                      if(i<bpduFields.GetPathVectorNumBridgeIds()-1) stringOut << ">";
                      numBytes = numBytes + 8;
                    }
                  stringOut << "] (TxHC:"  << m_portInfo[p].txHoldCount << ") (" << debugComment << ") (link: " << outPort->GetChannel()->GetId() << ") (bytes: " << numBytes << " )";
                  NS_LOG_INFO(stringOut.str());        
              ...
              else
                {
                  ...
                  NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Discarded BPDU to Send due to MessAge >= MaxAge [R:" << bpduFields.GetRootId() << "|C:" << bpduFields.GetCost() << "|B:" << bpduFields.GetBridgeId() << "|P:" << bpduFields.GetPortId() << "|A:" << bpduFields.GetMessAge() << "|Tc:" << bpduFields.GetTcFlag() << "|Tca:" << bpduFields.GetTcaFlag() << "]"); 
                }
            ...
            {
              NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] SPBDV: Holds BPDU tree " << tree << " (TxCount: " << m_portInfo[p].txHoldCount << " => " <<  m_portInfo[p].txHoldCount+1 << ")");
              m_portInfo[p].txHoldCount ++; // even if we have not sent, keep counting to differentiate 1 sent from 1 sent and 1, or more, hold
              m_portInfo[p].atLeastOneHold.at(tree)=true;
            } 
          m_portInfo[p].newInfo.at(tree)=false;
        }
    }  
}
                  ...
                  if(m_portInfo[i].role.at(t)==DESIGNATED_ROLE && m_bridgeInfo.activeTree.at(t)==true) //send a BPDU to each tree (except first set of messages)
                    {
                      ...
                      debugComment << "periodical_BPDU_tree_" << t;
                      SendBPDU(m_ports[i], debugComment.str() ,t);
                    } 
                }
            }
                  ...
                  if(m_portInfo[i].role.at(tree)==DESIGNATED_ROLE && m_bridgeInfo.activeTree.at(tree)==true) //send a BPDU to each tree (except first set of messages)
                    {
                      ...
                      SendBPDU(m_ports[i], debugComment.str() ,tree);
                    } 
                //}
            }
                      ...
                      if(m_portInfo[p].atLeastOneHold.at(t)==true && m_bridgeInfo.activeTree.at(t)==true)
                        {
                          ...
                          SendBPDU(m_ports[p], "pending_txHold", t);
      ...
      for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
        {
          if(m_portInfo[i].role.at(tree) == DESIGNATED_ROLE && m_bridgeInfo.activeTree.at(tree)==true)
            {
              m_portInfo[i].newInfo.at(tree)=true;
              SendBPDU(m_ports[i], "maxAge_expiration", tree);
            }
        }
    } 
}
