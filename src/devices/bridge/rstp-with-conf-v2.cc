    ...
    )
{
  ...
  for (NetDeviceContainer::Iterator i = ports.Begin (); i != ports.End (); ++i)
    {
      ...
      m_node->RegisterProtocolHandler (MakeCallback (&RSTPconf::ReceiveFromBridge, this), 9999, *i, true); // protocol = 0 (any, because we use the field to pass portId), promisc = false
  ...
  for (uint16_t i=0 ; i < m_portInfo.size () ; i++)
    {
      ...
      m_portInfo[i].toLearningTimer.SetFunction(&RSTPconf::MakeLearning,this);
      m_portInfo[i].toLearningTimer.SetDelay(Seconds(m_bridgeInfo.forwDelay));
      m_portInfo[i].toForwardingTimer.SetFunction(&RSTPconf::MakeForwarding,this);
      m_portInfo[i].toForwardingTimer.SetDelay(Seconds(m_bridgeInfo.forwDelay));
      m_portInfo[i].recentRootTimer.SetFunction(&RSTPconf::RecentRootTimerExpired,this);
      m_portInfo[i].recentRootTimer.SetDelay(Seconds(m_bridgeInfo.forwDelay));

      //configure topChange value
      m_portInfo[i].topChangeTimer.SetFunction(&RSTPconf::TopChangeTimerExpired,this);
      ...
      //if(m_rstpActivateExpirationMaxAge == 1)
        //{
          m_portInfo[i].messAgeTimer.SetFunction(&RSTPconf::MessageAgeExpired,this);
      ...
      m_portInfo[i].phyLinkFailDetectTimer.SetFunction(&RSTPconf::PhyLinkFailureDetected,this);
  ...
  m_bridgeInfo.txHoldCountTimer.SetFunction(&RSTPconf::ResetTxHoldCount,this);
  ...
  m_bridgeInfo.helloTimeTimer.SetFunction(&RSTPconf::SendHelloTimeBpdu,this);
  m_bridgeInfo.helloTimeTimer.SetDelay(Seconds(m_bridgeInfo.helloTime));
  m_bridgeInfo.helloTimeTimer.Schedule(Seconds(initTime));

  // variables to manage confirmations (RSTPConf)
  m_activateRootFailureConf=1;
  m_iAmNeighbor=false;
  m_numNeighbors=m_portInfo.size ();
  m_rdRcvdFromNeigh.resize(0);
  m_rdRcvdInPort.resize(0);
  m_neighId.resize(0);

}

////////////////////////////
// RSTPconf::ReceiveFromBridge //
////////////////////////////
void
RSTPconf::ReceiveFromBridge (Ptr<NetDevice> netDevice, Ptr<const Packet> pktBpdu, uint16_t protocol, Address const &src, Address const &dst, NetDevice::PacketType packetType)
{
  NS_LOG_FUNCTION_NOARGS ();
 
  //extract BPDU from packet
  BpduConfHeader bpdu = BpduConfHeader ();
  ...
  //////////////////////////////////
  if(bpdu.GetType()==BPDU_CONF_TYPE)   // if a confirmation BPDU is received
    {
      ...
      // trace reception
      NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] RSTP: Recvs CONF [R:" << bpdu.GetConfRootId() << "|N:" << bpdu.GetConfNeighId() << "|T:" << bpdu.GetConfType() << "] - (link id: " << m_ports[inPort]->GetChannel()->GetId() << ")");

      // transform neighId to local neighborId
      bool rdConfAlreadyRcvd=false;
      uint16_t indexNeigh=0;
      for(uint16_t i=0 ; i < m_neighId.size() ; i++)
        {
          if(m_neighId.at(i)==bpdu.GetConfNeighId())
            {
      ...
      ////////////////////////////
      if(bpdu.GetConfType()==0 || bpdu.GetConfType()==1)
        {
          if(bpdu.GetConfRootId()!=m_bridgeInfo.rootId)
            {
              NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[-] RSTP: CONF received in a node believing in another Root");              
            }
          else
            {
              // if already received
                  ...
                  if(m_bridgeInfo.rootId == m_bridgeInfo.bridgeId)
                    {
                        //////////////////////////////////////////////////////////////////
                        // RSTP-CONF-V2 
                        if(bpdu.GetConfType()==0) // if 'rd-with-reply'
                          {
                            SendConfirmationBpdu(m_ports[inPort], 2, m_bridgeInfo.rootId, bpdu.GetConfNeighId()); // send an 'ra'
                         
                            // mark the replied neighbour
                            m_neighId.push_back(bpdu.GetConfNeighId());
                            m_rdRcvdFromNeigh.push_back(true);
                            m_rdRcvdInPort.push_back(inPort);                               
                          }
                        if(bpdu.GetConfType()==1)  // if 'rd-without-reply'
                          {
                            // do nothing
                          }
...
/*
                        //////////////////////////////////////////////////////////////////
                        // RSTP-CONF-V1 
                        SendConfirmationBpdu(m_ports[inPort], 1, m_bridgeInfo.rootId, bpdu.GetConfNeighId()); // send an 'ra'
...
*/
                          ...
                          if(p!=inPort)
                            {
                              //////////////////////////////////////////////////////////////////
                              // RSTP-CONF-V2   
                              SendConfirmationBpdu(m_ports[p], bpdu.GetConfType(), bpdu.GetConfRootId(), bpdu.GetConfNeighId()); // send an 'rd?'
                            }
                        }
                      
                      // check if all 'rd?' have been received 
                      if(m_neighId.size()==m_numNeighbors)
                        {
                          NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[-] RSTP: All 'rd?' have been recieved. Must arise as Root now!");
                          
                          // clear info and arise as Root
                              ...
                              if(m_portInfo[i].state!=DISABLED_STATE)
                                {
                                  ...
                                  m_portInfo[i].designatedPortId = m_ports[i]->GetIfIndex();
                                  m_portInfo[i].messAge = 0;
                                  m_portInfo[i].receivedBpdu.rootId = m_bridgeInfo.rootId;;
                                  m_portInfo[i].receivedBpdu.cost = 0; 
                                  m_portInfo[i].receivedBpdu.bridgeId = m_bridgeInfo.bridgeId;
                                  m_portInfo[i].txHoldCount = 0;
                                  m_portInfo[i].topChange = false;
                                  m_portInfo[i].topChangeAck = false;
      ...
      ////////////////////////////
      else if(bpdu.GetConfType()==1) // received 'ra'
        {
          ...
          if(bpdu.GetConfNeighId() == m_bridgeInfo.bridgeId)
            {
              // clear variables
              m_neighId.erase(m_neighId.begin()+indexNeigh);
              m_rdRcvdFromNeigh.erase(m_rdRcvdFromNeigh.begin()+indexNeigh);
              m_rdRcvdInPort.erase(m_rdRcvdInPort.begin()+indexNeigh);
          ...
          else
            {
              ...
              uint16_t markedPort=m_rdRcvdInPort.at(indexNeigh);
              
              //////////////////////////////////////////////////////////////////
              // RSTP-CONF-V2   
              SendConfirmationBpdu(m_ports[markedPort], 2, bpdu.GetConfRootId(), bpdu.GetConfNeighId()); // send an 'ra'          
  ...
  ////////////////////////////
  else if(bpdu.GetType()==BPDU_TYPE) //if a common BPDU is received
    {
      //trace message
      NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << inPort << "] RSTP: Recvs BPDU [R:" << bpdu.GetRootId() << "|C:" << bpdu.GetCost() << "|B:" << bpdu.GetBridgeId() << "|P:" << bpdu.GetPortId() << "|A:" << bpdu.GetMessAge() << "|Tc:" << bpdu.GetTcFlag() << "|Tca:" << bpdu.GetTcaFlag() << "|" << bpdu.GetRoleFlags() << (bpdu.GetPropFlag()?"P":"-") << (bpdu.GetAgrFlag()?"A":"-") << (bpdu.GetLrnFlag()?"L":"-") << (bpdu.GetFrwFlag()?"F":"-") << "][Neigh:" << bpdu.GetNumNeighbors() << "] - (link id: " << m_ports[inPort]->GetChannel()->GetId() << ")");
              ...
              if(bpduUpdates == 1 && bpdu.GetRoleFlags()==DESIGNATED_ROLE)
                {
                  ...
                  // update the number of neighbors received (RSTPconf)
                  if(m_activateRootFailureConf==1)
                    {
                      m_numNeighbors = bpdu.GetNumNeighbors();
                    }
                  ...
                  if(m_activateRootFailureConf==1)
                    {
                      if(m_portInfo[m_bridgeInfo.rootPort].designatedBridgeId == m_bridgeInfo.rootId)
                        {
                          m_iAmNeighbor=true;
                          //NS_LOG_INFO ("TEST => Brdg[" << m_bridgeInfo.bridgeId << "] is a neighbor of Root " << m_bridgeInfo.rootId);                  
                        }
                      else
                        {
                          m_iAmNeighbor=false;
                          //NS_LOG_INFO ("TEST => Brdg[" << m_bridgeInfo.bridgeId << "] is not a neighbor of Root " << m_bridgeInfo.rootId);                    
                        }
                    }
...
//////////////////////////////
// RSTPconf::CheckProcessTcFlags //
//////////////////////////////
void
RSTPconf::CheckProcessTcFlags(bool tcaFlag, bool tcFlag, uint16_t port)
{
...
////////////////////
//RSTPconf::UpdatesInfo//
////////////////////
int16_t
RSTPconf::UpdatesInfo(bpduInfo_t bpdu, uint16_t port)
{
  NS_LOG_FUNCTION_NOARGS ();
...
RSTPconf::ReconfigureBridge()
{
  ...
  if(minRootId == m_bridgeInfo.bridgeId)   //if i'm the root, there is no need to look for a root port
    {
      ...
      m_bridgeInfo.cost=0;
      if(m_activateRootFailureConf==1) m_numNeighbors=m_portInfo.size ();
    }
  ...
  // print out Confirmation variables
  for(uint16_t i=0 ; i < m_neighId.size() ; i++)
    {
      NS_LOG_INFO(Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[-]       Conf Info  [N:" << m_neighId.at(i) << "|P:" << m_rdRcvdInPort.at(i) << "]");
    }  
...
///////////////////////////////////////////
// RSTPconf::AllDesignatedsRecentRootExpired //
///////////////////////////////////////////
bool
RSTPconf::AllDesignatedsRecentRootExpired()
{
...
//////////////////////////////////////////
// RSTPconf::SetSyncToAllRootAndDesignateds //
//////////////////////////////////////////
void
RSTPconf::SetSyncToAllRest(uint16_t port)
{
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
...
///////////////////////////////////////
// RSTPconf::AllRootAndDesignatedsSynced //
///////////////////////////////////////
bool
RSTPconf::AllRestSynced(uint16_t port)
{
  for(uint16_t p=0 ; p < m_portInfo.size() ; p++)
    {
...
////////////////////////////////
// RSTPconf::QuickStateTransitions //
////////////////////////////////
void
RSTPconf::QuickStateTransitions()
{
...
///////////////////
void
RSTPconf::SendBPDU (Ptr<NetDevice> outPort, std::string debugComment)
{
  NS_LOG_FUNCTION_NOARGS ();
          ...
          if(m_portInfo[p].txHoldCount < m_bridgeInfo.maxBpduTxHoldPeriod) //if no BPDUs send during last period, we can safley sent
            {
              ...
              // Add BPDU header
              BpduConfHeader bpduFields = BpduConfHeader ();
              ...
              bpduFields.SetNumNeighbors(m_numNeighbors);
...
RSTPconf::SendHelloTimeBpdu()
{
  if( /*m_bridgeInfo.rootId == m_bridgeInfo.bridgeId &&*/ Simulator::Now() < Time(Seconds(m_simulationtime))) // everyone sends periodical BPDUs
    ...
    // clear variables if Root
    if(m_bridgeInfo.bridgeId == m_bridgeInfo.rootId)
      {
        if(m_activateRootFailureConf==1)
          {
            m_neighId.resize(0);
...
///////////////////////////
// RSTPconf::ResetTxHoldCount //
///////////////////////////
void RSTPconf::ResetTxHoldCount()
{
...
//////////////////////////
// RSTPconf::MakeDiscarding //
//////////////////////////
void RSTPconf::MakeDiscarding (uint16_t port)
{
...
///////////////////////
// RSTPconf::MakeLearning //
///////////////////////
void RSTPconf::MakeLearning (uint16_t port)
{
...
/////////////////////////
// RSTPconf::MakeForwarding //
/////////////////////////
void RSTPconf::MakeForwarding (uint16_t port)
{
...
////////////////////////////
// RSTPconf::MessageAgeExpired //
////////////////////////////
void RSTPconf::MessageAgeExpired(uint16_t port)
{
...
//////////////////////////////////
// RSTPconf::PhyLinkFailureDetected //
//////////////////////////////////
void RSTPconf::PhyLinkFailureDetected(uint16_t port)
{
  ...
  if(Simulator::Now() < Time(Seconds(m_simulationtime)))
    {
      ...
      if(m_activateRootFailureConf==1 && m_portInfo[port].role==ROOT_ROLE && m_iAmNeighbor==true) // issue a message 'rd?'
        {

          NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << port << "] RSTP: Physical Link Failure Detected in a root port of a Neighbor");      
          
          //////////////////////////////////////////////////////////////////
          // RSTP-CONF-V2
          //
          
          bool nodeHasAlternates=false;
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
            {
              if(m_portInfo[i].role == ALTERNATE_ROLE)
                {
                  nodeHasAlternates=true;
                }
            }
            
          // if the node has alternate ports
          if(nodeHasAlternates==true)
            {
              // send 'rd-with-reply' to all ports
              for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
                {
                  if(i!=port)
                    {
                      //////////////////////////////////////////////////////////////////
                      // RSTP-CONF-V2 
                      SendConfirmationBpdu(m_ports[i], 0, m_bridgeInfo.rootId, m_bridgeInfo.bridgeId); // send an 'rd-with-reply'
                    }
                }
            }
          else // if the node has not altenate ports
            {
              // same as common operation with tree configuration
              ReconfigureBridge();
              QuickStateTransitions();
              for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
                {
              ...
              // flood the 'rd-witout-reply'
              for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
                {
                  if(i!=port)
                    {
                      SendConfirmationBpdu(m_ports[i], 1, m_bridgeInfo.rootId, m_bridgeInfo.bridgeId); // send an 'rd-without-reply'
                    }
                }
            }

          // set the own node as the first neighbor thinking the root is dead
          m_neighId.push_back(m_bridgeInfo.bridgeId);  // map itself to next index
          m_rdRcvdFromNeigh.push_back(true);  // mark itself as asking for a confirmation
          m_rdRcvdInPort.push_back(999);  // mark '999' because the 'rd?' is issued by itself
          ...
          for(uint16_t i=0 ; i < m_portInfo.size() ; i++)
            {
              if(i!=port)
                {
                  SendConfirmationBpdu(m_ports[i], 0, m_bridgeInfo.rootId, m_bridgeInfo.bridgeId); // send an 'rd?'
                }
            }
...
////////////////////////////////////
// RSTPconf::SendConfirmationBpdu //
////////////////////////////////////
void
RSTPconf::SendConfirmationBpdu(Ptr<NetDevice> outPort, uint16_t type, uint64_t rootId, uint64_t neighId)
{
// type: 'rd?'(0) | 'ra'(1)
  ...
  if(m_portInfo[p].state!=DISABLED_STATE)
    {
      ...
      // send to port
      NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << outPort->GetIfIndex() << "] RSTP: Sends CONF [R:" << bpduFields.GetConfRootId() << "|N:" << bpduFields.GetConfNeighId() << "|T:" << bpduFields.GetConfType()  << "] - (link id: " << outPort->GetChannel()->GetId() << ")");        
      outPort->SendFrom (bpdu, src, dst, protocol);   
    }  
}
...
////////////////////////////////
// RSTPconf::TopChangeTimerExpired //
////////////////////////////////
void RSTPconf::TopChangeTimerExpired(uint16_t port)
{
  if(m_rstpActivateTopologyChange==1)
    {
      NS_LOG_INFO (Simulator::Now() << " Brdg[" << m_bridgeInfo.bridgeId << "]:Port[" << port << "]      Port leaves Topology Change Procedure");
...
//////////////////////////////
// RSTPconf::ScheduleLinkFailure //
//////////////////////////////
void
RSTPconf::ScheduleLinkFailure(Time linkFailureTimeUs, uint16_t port)
{  
...
////////////////////////////////////
// RSTPconf::PrintBridgeInfoSingleLine //
////////////////////////////////////
void
RSTPconf::PrintBridgeInfoSingleLine()
{
