...
public:
  ...
  typedef struct{
    ...
    Timer                     lspTransmissionIntervalTimer;
    std::vector<uint64_t>     pendingLSPs;
  }portInfo_t;
  ...
  void SetSinglePortToForwarding(bool forw, uint16_t port);
...
private:
   ...
   */
  void SendLSP(uint16_t outPort, uint64_t lspNodeId);
  ...
  */
  ...
  double  m_lspTransmissionInterval; // minimum time between transmissions of the same LSP
  uint16_t m_activatePortActivationV1;
  uint16_t m_activatePortActivationV2;
