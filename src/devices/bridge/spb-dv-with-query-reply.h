...
#define BPDU_QUERY_REPLY 700 // BPDU used to query the root about its existence (before using information in an alternate)
...
public:
  ...
  typedef struct{
    ...
    std::vector<bool>         frozen;                     // frozen, in query-repl, mode avoids to use the info of that port
    std::vector<bool>         markedToReply;              // marked, in query-repl, because it has received a query, and it has to send a later reply
    std::vector<bool>         safeToUse;                  // in query-repl, the info is safeToUSe right after a reply+ is received
...
private:
  ...
  void SendQueryReply(Ptr<NetDevice> outPort, uint64_t tree, uint16_t qr);
  ...
  uint16_t  m_spbdvActivateQueryReply;            // true: use query-reply
